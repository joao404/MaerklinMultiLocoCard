...
{
  m_instance->m_transmissionState = TransmissionState::WaitingForRequest;
  plugCard();
}
...
void Smartcard::cyclic()
{
  ...
  switch(m_instance->m_transmissionState)
  {
    ...
    break;
    case TransmissionState::WaitingForRequest:
    break;    
    default:
...
void Smartcard::interruptHandler(i2c_inst_t *i2c, i2c_slave_event_t event)
{
    ...
    { 
      ...
      case I2C_SLAVE_REQUEST: // master is requesting data
        ...
        m_instance->m_memoryAddress++;
        m_instance->m_memoryAddress &= 0x1FFF;
        if(m_instance->m_transmissionState == TransmissionState::WaitingForRequest)
        {
      ...
      case I2C_SLAVE_FINISH: // master has signalled Stop / Restart
        ...
        if((m_instance->m_lastMemoryAddress <= m_instance->m_memoryAddress) && (m_instance->m_transmissionState == TransmissionState::TransmittingData))// overflow of address. Indicates complete reading of buffer
        {
          m_instance->m_transmissionState = TransmissionState::DelayOfTransmissionEnd;
        }
        break;
