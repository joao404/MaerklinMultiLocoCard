...
// Handling:
// plug card into MS2. Connect voltage
// You can immediatly start with transmitting locos
// If button is preset, transmission of locos is started
...
void setup()
{
  ...
  */
  ...
  ssd1306->drawString(0, 10, "Zum Lesen,");
  ssd1306->drawString(0, 20, "Knopf druecken");
  ssd1306->display();
  lococard->plugCard();
}
...
void loop()
{
  ...
  {
    // if button is pressed, reading is not blocked anymore
    Serial.println("Reading activated");
...
void handleLocoWritingFinished()
{
  ...
  else
  {
    ...
    ssd1306->drawString(0, 10, "Versuchs nochmal");
    ssd1306->display();
  }  
  memset(lococard->getMemory(), 0, 8192);  
}
